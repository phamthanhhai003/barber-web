  ...
  const handleSimulation = async (stylePrompt: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            { text: `Edit this image. Apply a ${stylePrompt}. Replace the current hairstyle with the one described. Keep the face, features, and person identical. Output ONLY the modified image in high quality black and white.` }
          ],
        },
        config: { 
      ...
      if (imagePart) {
        ...
        throw new Error("AI không trả về kết quả ảnh. Vui lòng thử lại.");
      }
    } catch (error: any) {
      console.error(error);
      setErrorStatus(`LỖI: ${error.message.substring(0, 100)}`);
    } finally {
      setLoading(false);
    }
  };
